  ...
  return (
    <div className="relative h-full flex flex-col bg-white dark:bg-gray-800 rounded-3xl overflow-hidden border border-gray-100 dark:border-gray-700 shadow-sm group">
      ...
      <div className="absolute top-4 left-4 right-4 z-20 flex justify-between items-center pointer-events-none">
        <h3 className="text-[12px] font-bold uppercase tracking-widest text-white/90 drop-shadow-md">
          {lang === 'es' ? 'Proyectos' : 'Projects'}
        </h3>
        <a
          href={lang === 'es' ? '/es/projects' : '/en/projects'}
          className="pointer-events-auto text-[12px] font-bold text-white/80 hover:text-white bg-black/20 hover:bg-black/40 backdrop-blur-sm px-2 py-1 rounded-full transition-all flex items-center gap-1"
        >
      ...
      <div className="relative h-64 overflow-hidden bg-gray-100 dark:bg-gray-900">
        {p.image ? (
          <img src={p.image} alt={p.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" width={"256"} height={"256"} fetchPriority="high" />
        ) : (
      ...
      <div className="p-4 flex flex-col flex-grow justify-between">
        <p className="text-[12px] leading-relaxed text-gray-600 dark:text-gray-400 line-clamp-3 mb-4">
          {p.shortDescription || p.description}
        </p>

        <div className="flex items-center justify-between mt-auto">
          <div className="flex items-center gap-3">
            <a href={detailsLink} className="px-3 -ml-3 text-[11px] uppercase tracking-widest font-bold text-blue-600 dark:text-blue-400 hover:underline rounded-lg transition-all">
              {lang === 'es' ? 'Detalles' : 'Details'}
            </a>

            <div className="flex gap-2 border-l border-gray-100 dark:border-gray-700 pl-3">
              {p.githubLink && (
                <a href={p.githubLink} target="_blank" className="p-2 text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors" title="GitHub">
              ...
              {p.demoLink && (
                <a href={p.demoLink} target="_blank" className="p-2 text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors" title="Demo">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="p-2 rounded-lg bg-gray-50 dark:bg-gray-900 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors border border-gray-100 dark:border-gray-700 flex items-center justify-center"
            >
