...
export default function PdfCropper() {
  ...
  }, [pdfBytes, selected, pageCount, cropRects, pushUndo]);
  ...
  const handleDeleteOne = useCallback(async (idx:number)=>{
    ...
    setIsProcessing(false);
  }, [pdfBytes, pageCount, cropRects, pushUndo]);
      ...
      {pdfBytes && (
        <div className="mt-8 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <button onClick={async()=>{ if(!originalBytes) return; const b=cloneBytes(originalBytes); setPdfBytes(b); setCropRects(new Map()); setSelected(new Set()); setUndoStack([]); setRedoStack([]); }} className="px-4 py-3 rounded-xl border bg-white text-sm font-semibold">↺ Volver al original</button>
          </div>
