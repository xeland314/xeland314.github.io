    ...
    <div className="flex items-center justify-center overflow-hidden font-sans w-full h-full">
      <div ref={wrapperRef} className="relative">
        <div
          ...
          className={`relative overflow-hidden flex flex-col items-center ${showImage ? "justify-start p-14" : "justify-center p-12"} ${s.bg} ${s.text} shadow-2xl origin-top-left`}
          style={{
        ...
        >
          ...
          <div className={`relative z-10 flex flex-col items-center text-center ${showImage ? "w-full h-full pb-32" : "max-w-5xl"}`}>
            {/* Category Badge */}
            <div className={`mb-${showImage ? "8" : "10"} relative group`}>
              ...
              <div className={`relative px-8 py-3 rounded-full border-2 ${c.border}/40 ${s.bg || "bg-white/10"} backdrop-blur-md shadow-xl shrink-0`}>
                <span className={`${c.textAccent} font-bold tracking-[0.3em] uppercase ${showImage ? "text-xl" : "text-2xl"} font-mono flex items-center gap-3`}>
            ...
            {/* Title */}
            <h1 className={`${showImage ? "text-6xl" : "text-8xl"} font-inter font-black text-transparent bg-clip-text bg-gradient-to-br ${c.gradient} mb-${showImage ? "10" : "8"} tracking-tight leading-tight max-w-4xl shrink-0 drop-shadow-2xl`}>
              {title}
            </h1>
            ...
            {showImage ? (
              ...
              <>
                ...
                <div className="mb-14 relative group">
                  <div className={`absolute inset-0 bg-gradient-to-tr ${c.gradient} rounded-[3rem] blur-2xl opacity-40 group-hover:opacity-60 transition-opacity duration-500`} />
                  <div className={`relative p-1.5 rounded-[3rem] bg-gradient-to-tr ${c.gradient} shadow-2xl transform hover:scale-105 transition-transform duration-500`}>
                    <div className={`relative w-48 h-48 rounded-[2.75rem] ${s.bg || "bg-black/40"} flex items-center justify-center backdrop-blur-sm overflow-hidden`}>
                ...
                {/* Gradient Divider */}
                <div className={`h-2 w-32 rounded-full bg-gradient-to-r ${c.gradient} mb-10 opacity-80 shadow-lg`} />
              </>
            )}
              ...
              <div className="flex items-center gap-4">
                ...
                {theme.showLogo && (
                  <div className={`p-1.5 rounded-xl bg-gradient-to-tr ${c.gradient} shadow-md`}>
                    <img src={theme.logoImage} alt="Logo" className={`${showImage ? "w-10 h-10" : "w-20 h-20"} rounded-lg object-cover border border-white/20`} loading="lazy" />
                  </div>
                )}
              </div>
            </div>
